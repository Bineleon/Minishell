...
/*   Updated: 2024/12/12 16:13:28 by neleon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *init_res2(void)
{
    char *result = gc_mem(MALLOC, 1, NULL);
    result[0] = '\0';
    return (result);
}
...
{
    char *var_name = gc_mem(MALLOC, end - start + 1, NULL);
...
int process_env_var(char **result, char *str, int i, t_env *env_list)
{
    ...
    if (str[i] == '?')
    {
        char *exit_status = expand_exit_status();
    ...
    while (str[i] && (ft_isalnum(str[i]) || str[i] == '_'))
        i++;
    char *var_name = extract_var_name2(str, start, i);
